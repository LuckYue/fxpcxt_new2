...
public class UploadFaceActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mSfview = (SurfaceView)findViewById(R.id.surface_view1);
        btn_surfaceLogin=findViewById(R.id.surface_login1);
...
//                mFrame = (FrameLayout)findViewById(R.id.frame);
                mPreview = new CameraPreview(UploadFaceActivity.this,mcamera);
               // mPreview = new FaceJudgeActivity.CameraPreview(UploadFaceActivity.this,mcamera);
...
//            }
    ...
    public void faceLogin(File file){
        new Thread(new Runnable() {
            @Override
            public void run() {
                String url="http://192.168.43.200:7001/face/addUser";
                try {
                    login(url,file);
                } catch (IOException e) {
                    e.printStackTrace();
            ...
            if(response.isSuccessful()){
                ...
                if(commonRespones.isSuccess()){
                    this.runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            Toast.makeText(getBaseContext(), "人脸注册成功", Toast.LENGTH_SHORT).show();
                    ...
                    this.runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            Toast.makeText(getBaseContext(), "人脸注册失败", Toast.LENGTH_SHORT).show();
                        }
                    });
                }
